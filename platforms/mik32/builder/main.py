...
upload_protocol = board.get(f"debug.protocol", {}) #env.subst("$UPLOAD_PROTOCOL")
debug_tools = board.get(f"debug.tools", {})
upload_actions = []
...
tool_args.extend(debug_tools.get(upload_protocol).get("server").get("arguments", []))
#tool_args.extend(debug_tools.get(upload_protocol).get("server").get("arguments", []))


ldscript = board.get(f"debug.ldscript")
hex_path = target_hex[0].rstr().replace('\\', '/')
command = ("eeprom_write_file %s" % hex_path) \
    if ldscript == "eeprom" \
    else "load_image %s %s ihex" % (hex_path, board.get("upload").get("image_offset", "0x0"))
tool_args.extend(
    [
        "-c", "reset halt",
        "-c", command,
        "-c", "resume %s" % board.get("upload").get("image_offset", "0x0"),
        "-c", "shutdown"
    ]
)
